...
func (t *Task) AddCourse(course string) error {
	...
	response, err := t.DoReq(t.MakeReq("GET", fmt.Sprintf("https://reg-prod.ec.fhda.edu/StudentRegistrationSsb/ssb/classRegistration/addRegistrationItem?term=%s&courseReferenceNumber=%s&olr=false", t.Term, course), headers, nil))
	if err != nil {
		discardResp(response)
		return err
	}
...
func (t *Task) SendBatch() error {
	...
	response, err := t.DoReq(t.MakeReq("POST", "https://reg-prod.ec.fhda.edu/StudentRegistrationSsb/ssb/classRegistration/submitRegistration/batch", headers, []byte(string(batchJson))))
	if err != nil {
		discardResp(response)
		return err
	}
		...
		for _, courseReferenceNumber := range t.CRNs {
			if data.CourseReferenceNumber == courseReferenceNumber {
				if data.StatusDescription == "Registered" {
					t.Status = "Successfully Registered"
				} else if data.StatusDescription == "Waitlisted" {
					t.Status = "Successfully Waitlisted"
